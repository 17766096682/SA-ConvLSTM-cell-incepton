...
class SA_Convlstm_cell(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        combined_conv_3x3 = self.conv2d_3x3(combined)
        ii, ff, oo, gg = torch.split(combined_conv, self.hidden_dim, dim=1)
        i_, f_, o_, g_ = torch.split(combined_conv_3x3, self.hidden_dim, dim=1)

        i,f,o,g = ii+i_,ff+f_,oo+o_,gg+g_
        i = torch.sigmoid(i)
        f = torch.sigmoid(f)
        o = torch.sigmoid(o)
        g = torch.tanh(g)
